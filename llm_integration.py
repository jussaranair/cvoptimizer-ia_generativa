...
def generate_missing_keywords_llm(resume_text: str, temperature: float = 0.2, max_tokens: int = 256) -> Optional[List[str]]:
	...
	try:
		...
		content = response.json()["choices"][0]["message"]["content"]
		import json, re
		# Tenta extrair lista JSON de qualquer parte do texto
		json_match = re.search(r'\[.*\]', content, re.DOTALL)
		if json_match:
			try:
				return json.loads(json_match.group(0))
			except Exception as e:
				print("[ERROR] Falha ao carregar lista JSON extraída:", e)
				return [content]
		else:
			print("[WARN] Resposta não contém lista JSON. Retornando texto bruto.")
			return [content]
	except Exception as e:
		print("[ERROR] Exception ao chamar Groq:", e)
		return [str(e)]
